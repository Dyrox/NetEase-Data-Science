...
import json
import os
from modules.GenerateLinksToListOfPlaylists import link_gen
import time
import random
...
class Netease_spider:
    ...
    def parse4data(self,html):
        ...
        for index, item in enumerate(play_num):
            if '万' in item:
                play_num[index] = item.replace('万','0000')
...
for key,value in LINKS_DICT.items():
    ...
    print(f"Processing {key} with URL: {value}")
    spider.crawl()
